...
class HealthDataExtractor(object):
    ...
    """
    def __init__(self, dest_path, download_string, dest_bucket, verbose=VERBOSE):
        # self.in_path = path
        ...
        #     self.report('done')
        self.data = ElementTree.fromstring(download_string)
        os.remove(path)
    ...
    def close_files(self):
        for (kind, f) in self.handles.items():
            blob = self.dest_bucket.blob("{dest_path}/{name}.csv".format(dest_path = self.dest_path, name = abbreviate(kind)))
            # blob = self.dest_bucket.blob('%s.csv' % abbreviate(kind))
            path = os.path.join(self.directory, '%s.csv' % abbreviate(kind))
...
def applehealthcsv(event, context):
    ...
    download_string = blob.download_as_string()
    ...
    data = HealthDataExtractor(dest_path, download_string, dest_bucket)
    # data.report_stats()
    data.extract()
